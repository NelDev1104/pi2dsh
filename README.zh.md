...
| 档位 | 数量 | 含义 |
|---|---|---|
| ✅ **已测可用** | **34 / 50** | 在真实 DSH runtime 挂载且**真实执行**验证：30 个成功返回，4 个业务逻辑端到端真跑（拒绝了合成探针参数）；其中 4 个另通过深度验证：真 LSP 子进程、真搜索/抓取、PNG 真落盘、官方 `dsh plugin` 安装/激活/卸载全流程 |
