...
## 上游报账（2026-08-31 起：先对账、用户确认后才发）

| 项 | 状态 |
|---|---|
| [pi#8858](https://github.com/earendil-works/pi/issues/8858) 围栏参数静默降级 | **已发**（发在立规之前；用户裁定"发俩就算了"，留着不动） |
| [pi#8861](https://github.com/earendil-works/pi/issues/8861) 同 index 吞第二个调用 | **已发**（同上） |
| body 字段透传缺失（`user` 等，消费者 #3761/#599） | **攒着待对账**，不发 |
| 后续所有上游 issue/PR | 一律攒进本表，对账确认后才发 |
