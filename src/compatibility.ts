...
export const CONTEXT_RULES: Readonly<Record<string, SurfaceRule>> = Object.freeze({
  ...
  hasUI: surface('full', 'Reports whether a real interactive surface exists: true for a mounted public terminal surface (dsh-TUI tuiScenes or dsh-pi-tui piTuiExtensions) or for a DSH user-question provider; false for a genuinely headless composition and for child-agent questions that DSH refuses.', 'The terminal answer comes from the capability-selected TerminalSurfaceAdapter. The question answer is probed rather than guessed: the bridge registers a throwaway UserQuestion provider and reads the documented DUPLICATE_PROVIDER rejection as "a real provider is already there", then disposes it. Inside a child agent only the terminal seat can make this true, because DSH refuses child-agent questions.'),
  mode: surface('full', 'Reports tui whenever a full-screen custom seat exists — dsh-TUI scenes, dsh-pi-tui raw mounts, or (browserPresentation on, as the dsh-work-x suite configures) the web scene seat — and rpc otherwise. An engine-only web composition is rpc on purpose: the engine ships no web renderer, so packages degrade exactly as they do headless. Pi packages read mode as "can I show interactive UI here?".', 'Derived from the live optional TerminalSurfaceAdapter, else from the browser half\'s routed scene seat — routed only under the engine\'s browserPresentation config (default off; the renderer is a product layer\'s to carry). Surface selection is by public service/capability (`tuiScenes` or `piTuiExtensions` + unstable.surface.handle), never by Pi consumer package.'),
...
export const UI_CONTEXT_RULES: Readonly<Record<string, SurfaceRule>> = Object.freeze({
  ...
  custom: surface('partial', 'Runs the real Pi component on every interactive front door: a dsh-TUI full-screen scene, dsh-pi-tui\'s raw component mount, or the Web overlay. Width, ANSI frames, raw input, render invalidation, result completion and disposal are preserved. Only genuinely headless compositions resolve undefined, Pi\'s own rpc-mode behavior — and product-UI command invocations (a side-chat window running a package command it presents itself) take the same official degradation so the package\'s component never doubles the window.', 'The Pi component stays Host-owned behind a transport-neutral relay whose messages are sessionId/width/lines/input/revision/close. The current local dsh-pi-tui transport wraps that relay with the public UNSTABLE_API_LEVEL=1 mountComponent facade; dsh-TUI and Web keep their public scene seats. No callback or component object is part of the relay contract, so a future DSH Server/Client split changes only the transport. One scene at a time, opening a new one finishes the previous run.'),
