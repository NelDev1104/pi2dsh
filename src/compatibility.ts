...
export const UI_CONTEXT_RULES: Readonly<Record<string, SurfaceRule>> = Object.freeze({
  ...
  setStatus: surface('partial', 'Pi\'s keyed status entries render as pills in the bridge\'s own browser half (Pi\'s status bar is a terminal-footer surface; DSH\'s equivalent is the frame-wide pill stack). setStatus(key, undefined) removes exactly one entry, Pi\'s clear shape.', 'BrowserSurfaces.setStatus keys entries by (session, package, status key); the client half polls the bridge\'s own /pi2dsh/browser-state route and draws them in its shell.overlay seat. Pi\'s rpc mode transmits setStatus to a host too — this is one of the surfaces rpc genuinely wires.'),
