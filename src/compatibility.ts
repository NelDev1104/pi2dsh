...
export const CONTEXT_RULES: Readonly<Record<string, Rule>> = Object.freeze({
  ...
  modelRegistry: rule('partial', 'A live registry over the ONE model directory: every Pi provider (package-registered or ~/.pi/agent/models.json) is a DSH llm route, and the registry is that directory\'s exact Pi projection — Pi-native entries keep api/baseUrl and the full Model shape. getProviderAuth/getApiKeyAndHeaders run Pi\'s full credential chain (models.json keys resolve with Pi\'s $ENV/!command semantics); DSH-owned routes keep their credentials inside their adapters by design. A models.json entry carrying ONLY modelOverrides that declare input:["text","image"] for an existing route registers a "<route>-vision" image-admission companion: a real DSH route that admits images, replaces image blocks with an explicit text notice, and forwards to the original route (Pi itself has no admission layer, so this projection is bridge-defined). Stored attachments are materialized to temp files and the notice names the path — Pi\'s image vocabulary is inline-or-path, so a path lets any path-taking tool (Pi\'s read, vision tools) reach the image.'),
