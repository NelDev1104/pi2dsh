...
export const CONTEXT_RULES: Readonly<Record<string, SurfaceRule>> = Object.freeze({
  ...
  mode: surface('full', 'Reports tui whenever a full-screen custom seat exists — dsh-TUI scenes, dsh-pi-tui raw mounts, or the bridge\'s web scene seat — and rpc only in headless compositions. Pi packages read mode as "can I show interactive UI here?", and each of those seats can.', 'Derived from the live optional TerminalSurfaceAdapter, else from the browser half\'s routed scene seat. Surface selection is by public service/capability (`tuiScenes` or `piTuiExtensions` + unstable.surface.handle), never by Pi consumer package.'),
