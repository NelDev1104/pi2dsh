...
export const CONTEXT_RULES: Readonly<Record<string, SurfaceRule>> = Object.freeze({
  ...
  mode: surface('full', 'Reports tui whenever a full-screen custom seat exists — dsh-TUI\'s scene service in the terminal, the bridge\'s web scene seat in a browser composition — and rpc only in headless compositions. Pi packages read mode as "can I show interactive UI here?", and on the web the answer is yes.', 'Derived from the live optional TuiSurfaceAdapter, else from the browser half\'s routed scene seat. This is the branch interactive Pi plugins take before calling ui.custom; it changes with the Cordis service lifecycle rather than being a constant.'),
...
export const UI_CONTEXT_RULES: Readonly<Record<string, SurfaceRule>> = Object.freeze({
  ...
  custom: surface('partial', 'In dsh-TUI, runs the real Pi component in a native full-screen scene with raw terminal input, render invalidation, result completion and disposal. In a browser composition the SAME component runs on the web overlay: its ANSI frames paint in a modal, the browser keyboard forwards as raw terminal sequences, the browser\'s character width reaches render(width), the component\'s own done() resolves the Pi caller with its value, and dismissing the overlay resolves undefined — exactly like closing a terminal scene. Only headless compositions resolve undefined, Pi\'s own rpc-mode behavior.', 'The bridge adapts the public Pi component protocol (render(width), handleInput(raw bytes), requestRender, dispose) onto dsh-TUI\'s public tuiScenes seam when the terminal owns the screen, else onto the bridge\'s own browser scene seat: the server half holds the live component and serves its frames through the bridge\'s own route, the client half (a normal dsh.client slot plugin) paints the ANSI and posts keyboard sequences back. One scene at a time, opening a new one finishes the previous run — terminal semantics on both seats.'),
