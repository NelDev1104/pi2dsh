...
export const HOST_IMPORT_RULES: Readonly<Record<string, Readonly<Record<string, Rule>>>> = Object.freeze({
  'pi-coding-agent': Object.freeze({
    ...
    createAgentSession: rule('partial', 'Bridged to a genuine DSH child agent through ctx.agents with the creator-exclusive setup contribution: the parent\'s agent preset composes the host-native toolset into the child scope, the Pi tools allowlist/excludeTools become official scoped restrictions, customTools register in the child scope, delegation policy (sandbox override, approval pinned) travels the official way, and a Pi model on the options routes the child. Compositions without a loop factory fail explicitly.'),
