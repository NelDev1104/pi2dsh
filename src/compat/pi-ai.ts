...
function unbridgedTransport(api: string): never {
  throw new Error(`pi2dsh: the ${api} protocol client is not bridged in this composition (real pi-ai is not installed); model transports stay native to DSH llm/credentials`)
}

// Wire-protocol transports come from the REAL pi-ai when the host bundle
// installs it (host bundles provide the peer exactly as a Pi host would; the
// package-facing import surface stays on this shim via the jiti alias).
// Without it the explicit-failure contract stands.
let realPiAiPromise: Promise<Record<string, unknown> | undefined> | undefined
function loadRealPiAi(): Promise<Record<string, unknown> | undefined> {
  realPiAiPromise ??= import('@earendil-works/pi-ai').then(
    module => module as unknown as Record<string, unknown>,
    () => undefined,
  )
  return realPiAiPromise
}
...
/**
 * The REAL pi-ai's builtin provider definition for one id, when the host
 * bundle carries pi-ai. DSH's own llm adapter builds its directory from the
 * same builtinProviders(), so this is the DSH model layer's own source of
 * provider auth semantics (env-resolved API keys included).
 */
export async function realBuiltinProvider(providerId: string): Promise<UnknownRecord | undefined> {
  const real = await loadRealPiAi()
  if (real === undefined) return undefined
  try {
    const providersModule = await import('@earendil-works/pi-ai/providers/all').catch(() => real)
    const factory = (providersModule as UnknownRecord).builtinProviders ?? real.builtinProviders
    if (typeof factory !== 'function') return undefined
    const providers = (factory as () => Array<UnknownRecord>)()
...
export function openAIResponsesApi(): unknown {
  return lazyApiForCompat(async () => realTransport('openai-responses', 'openAIResponsesApi'), { fetchDeferred: true, cancelDeferred: true })
}

export function anthropicMessagesApi(): unknown {
  return lazyApiForCompat(async () => realTransport('anthropic-messages', 'anthropicMessagesApi'), undefined)
}
