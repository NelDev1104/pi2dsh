...
| [`custom`](#custom-ctxui) | `ctx.ui.*` | Mapped, difference stated | Runs the real Pi component on every interactive front door: a dsh-TUI full-screen scene, dsh-pi-tui's raw component mount, or the Web overlay. Width, ANSI frames, raw input, render invalidation, result completion and disposal are preserved. Only genuinely headless compositions resolve undefined, Pi's own rpc-mode behavior — and product-UI command invocations (a side-chat window running a package command it presents itself) take the same official degradation so the package's component never doubles the window. |
