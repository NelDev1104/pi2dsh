...
| [`custom`](#custom-ctxui) | `ctx.ui.*` | Mapped, difference stated | In dsh-TUI, runs the real Pi component in a native full-screen scene with raw terminal input, render invalidation, result completion and disposal. In a browser composition the SAME component runs on the web overlay: its ANSI frames paint in a modal, the browser keyboard forwards as raw terminal sequences, the browser's character width reaches render(width), the component's own done() resolves the Pi caller with its value, and dismissing the overlay resolves undefined — exactly like closing a terminal scene. Only headless compositions resolve undefined, Pi's own rpc-mode behavior. |
