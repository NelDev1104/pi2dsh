...
1. There's currently no channel for an out-of-repo plugin to append custom session events safely: `Session.append()` can't set the envelope's `ignorable: true`, and `KNOWN_SESSION_EVENT_TYPES` is an in-repo generated list — so we persist Pi custom entries in a sidecar instead of the main log. The session README defers a registration surface "until such a consumer exists"; pi2dsh is that consumer.
2. The official MCP client bridges tools only; resources/prompts have no consumption path yet.

Everything above is backed by machine-readable evidence in the repo (`community/*.json`), including a live acceptance run where `deepseek-v4-flash` calls a migrated Pi tool and the durable log is asserted. Feedback and gap reports welcome — the goal is that the remaining packages get to mountable too.
